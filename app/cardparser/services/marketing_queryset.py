...
from cardparser.models import TgUserProduct, ParseProduct, Brand, Category


def get_category_and_its_top_products(
    hours: int = 24,
    days: int = 0,
    ...
    """
    Возвращает самую активную категорию по количеству уникальных запрошенных товаров
    и топ-N самых запрашиваемых товаров этой категории.
    ...
    :return: Словарь с ключами "category" и "top_products" или пустой словарь
    """
    time_threshold = timezone.now() - timedelta(days=days, hours=hours)

    # 1. Уникальные product_id, запрошенные за период
    requested_product_ids = (
        TgUserProduct.objects.filter(sent_at__gte=time_threshold)
    ...
    # 2. Находим категорию с наибольшим количеством уникальных запрошенных товаров
    category_stat = (
        ParseProduct.objects.filter(
            id__in=requested_product_ids, category__isnull=False
        )
        .select_related("category")
        .exclude(category_id__in=exclude_category_ids or [])
        .values("category_id")
        .annotate(
    ...
    # 3. Получаем объект категории
    category = Category.objects.get(id=category_stat["category_id"])

    category_data = {
        "id": category.id,
        "name": category.name,
        "product_count": category_stat["product_count"],
        "last_sent": category_stat["last_sent"],
    }

    # 4. Получаем топ товаров ЭТОЙ категории по количеству запросов
    top_products_stats = (
        TgUserProduct.objects.filter(
            sent_at__gte=time_threshold,
            product_id__in=ParseProduct.objects.filter(
                category_id=category.id
            ).values_list("id", flat=True),
        )
        .values("product_id")
        .annotate(request_count=Count("product_id"), last_sent=Max("sent_at"))
        .order_by("-request_count", "-last_sent")[:limit]
    )

    if not top_products_stats:
        return {"category": category_data, "top_products": []}
    ...
    # 7. Сортируем, чтобы сохранить порядок из БД
    top_products.sort(key=lambda x: (-x["request_count"], -x["last_sent"].timestamp()))

    return {"category": category_data, "top_products": top_products[:limit]}
...
def get_brand_and_its_top_products(
    ...
    brand_stat = (
        ParseProduct.objects.filter(id__in=requested_product_ids, brand__isnull=False)
        .select_related("brand", "category")
        .exclude(category_id__in=exclude_category_ids or [])
        .values("brand_id")
        .annotate(
    ...
    top_products_stats = (
        TgUserProduct.objects.filter(
            sent_at__gte=time_threshold,
            product_id__in=ParseProduct.objects.filter(brand_id=brand.id).values_list(
                "id", flat=True
            ),
        )
        .values("product_id")
    ...
    return {"brand": brand_data, "top_products": top_products[:limit]}


def get_popular_products(hours: int = 24, days: int = 0, limit: int = 10):
    """
    ...
    products = (
        ParseProduct.objects.filter(id__in=stats_map.keys())
        .select_related("brand", "category")
        .only(
            "id",
            "product_id",
