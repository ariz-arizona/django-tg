...
REDIS_KEY_TEMPLATE = "user:ttl:{user_id}:{category}:{app_id}"

# Персональный кулдаун участника в конкретной группе
REDIS_GROUP_KEY_TEMPLATE = "group:user:ttl:{chat_id}:{user_id}:{category}:{app_id}"

# Шаблон для ID служебных сообщений в группе
